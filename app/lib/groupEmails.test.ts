...
import { decodeRoomKey } from "./roomKey";
...
describe("groupEmailsByRoom", () => {
  it("単一相手の基本グルーピング", () => {
    ...
    const groups = groupEmailsByRoom(emails, ME, {});
    expect(Object.keys(groups)).toEqual(["田中太郎"]);
    expect(groups["田中太郎"]).toHaveLength(1);
  });
  ...
  it("表示名ルームとアドレスルームが同一人物として統合される", () => {
    ...
    const groups = groupEmailsByRoom(emails, ME, {});
    expect(Object.keys(groups)).toEqual(["田中太郎"]);
    expect(groups["田中太郎"].map(e => e.id).sort()).toEqual(["1", "2"]);
  });
  ...
  it("送信済みメールが正しい相手の部屋に統合される", () => {
    ...
    const sent = groups["田中太郎"].find(e => e.id === "2")!;
  ...
  it("通常グループ（アドレスベース）が受信・送信双方を集約する", () => {
    ...
    expect(groups["group:1"].map(e => e.id).sort()).toEqual(["1", "2"]);
    // 一斉送信メールはメンバー個別チャットにも反映される
    expect(groups["田中太郎"].map(e => e.id)).toContain("2");
  });
  ...
  it("フィルターグループが条件に一致する受信メールのみを集約する（送信済みは常に除外）", () => {
    ...
    const groups = groupEmailsByRoom(emails, ME, chatConfigs);
    expect(groups["filter:1"].map(e => e.id)).toEqual(["1"]);
  });
});
...
describe("mergeAccountGroups", () => {
  it("2アカウントに同姓同名の相手がいても、roomKeyが異なるため別チャットとして分離される", () => {
    ...
    expect(merged[roomA].map(e => e.id)).toEqual(["a1"]);
    expect(merged[roomB].map(e => e.id)).toEqual(["b1"]);
  });
});
